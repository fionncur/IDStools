...
# ------------------------------------------------------------------------------------
def ids_get(idsname,shot,run,user_or_path,database,occ=0):
    ...
    return ids
# ------------------------------------------------------------------------------------
def ids_get_slice(idsname,time_slice,shot,run,user_or_path,database,occ=0,interp_method=1):
    input = imas.DBEntry(imas.imasdef.MDSPLUS_BACKEND,database,shot,run,user_or_path)
    input.open()
    ...
    return ids
# ------------------------------------------------------------------------------------
def ids_put(ids,shot,run,user_or_path,database,occ=0):
...
# ------------------------------------------------------------------------------------
def ids_put_slice(ids,shot,run,user_or_path,database,occ=0):
    output = imas.DBEntry(imas.imasdef.MDSPLUS_BACKEND,database,shot,run,user_or_path)
    retstatus,idx = output.open()
    if retstatus == 0:
        print('IDS appended to existing '+str(shot)+'/'+str(run)+'/'+user_or_path+'/'+database+' datafile')
