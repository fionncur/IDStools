...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Times IDS getting of given Pulse and Run, contrasts with IDS size and slices",
        parents=[imasParser],
    )
    parser.add_argument(
        "-md",
    ...
    parser.add_argument(
        "-p", "--pulse", type=int, help="Pulse number harboring desired IDSs"
    )
    ...
    parser.add_argument(
        "-exci", "--excludeids", nargs="*", type=str, help="IDS(s) to exclude"
    )
    parser.add_argument(
        "-rot",
        "--rotation",
    ...
    parser.add_argument(
        ...
        type=str,
        default="bold",
        help="Fontweight for plot axes (options include: 'ultralight', 'light', 'normal', 'bold', 'heavy') (default='bold')",
    )
    parser.add_argument(
        "-siz",
        "--sizevtime",
    ...
    parser.add_argument(
        "-xlog", "--xlogscale", action="store_true", help="Scale x-axis with logscale"
    )
    parser.add_argument(
        "-ylog", "--ylogscale", action="store_true", help="Scale y-axis with logscale"
    )
    parser.add_argument(
        "-sli",
        "--slicevtime",
    ...
    if not args.file:
        if args.database:
            mdslocpath = DBMaster.getDBPath(args.user, args.database, args.version)
            hdflocpath = mdslocpath
        elif args.mdsdatabase and args.hdf5database:
            mdslocpath = DBMaster.getDBPath(
                args.mdsuser, args.mdsdatabase, args.version
            )
            hdflocpath = DBMaster.getDBPath(
                args.hdf5user, args.hdf5database, args.version
            )
        else:
            raise Exception(
                "Please clarify where the data is stored. Please provide either one database containing data in both MDSPLUS and HDF5, or two separate databases containing data in MDSPLUS or HDF5"
            )
        ...
        if args.run:
            ...
            hpulses = DBMaster.hdf5ListPulseRun(hdflocpath)
            mpulses = DBMaster.mdsListPulseRun(mdslocpath)
            if set(hpulses) != set(mpulses):
                ...
                print(
                    "WARNING: The provided database(s) do(es) not have the same Pulses/Runs in both backends. The intersection of the two will be used."
                )
                if args.verbose:
                    print(
                        """The differences are:
        ...
        for entry in track(pulses, description="[green]Processing..."):
            ...
            for ids in track(idslist, description="[green]Processing..."):
                ...
                if args.slicevtime:
                    try:
                        slices.append(
                            get_timings(hdfde, idsname, times=[10], repeat=args.repeat)
                        )
                    except Exception:
                        slices.append(None)
                        print(
                            ...
                            + ", "
                            + run
                            + ") failed to return slice get_timings"
                        )
        ...
        if args.sizevtime:
            totaldf["MDSPLUS_PERFORMANCE (MB/s)"] = totaldf.apply(
                lambda row: (row.SIZE / row.MDSPLUS_TIME), axis=1
            )
            totaldf["HDF5_PERFORMANCE (MB/s)"] = totaldf.apply(
                lambda row: (row.SIZE / row.HDF5_TIME), axis=1
            )
        ...
        if args.slicevtime:
            ...
            totaldf["HDF5_PERFORMANCE (Slices/s)"] = totaldf.apply(
                lambda row: (row.SLICES / row.HDF5_TIME), axis=1
            )

        date = datetime.now().strftime("%Y_%m_%d-%I:%M:%S_%p")
        totaldf.to_csv(
            "totaldf-" + date + ".csv", na_rep="None", index=False, header=True
        )
